  ...
  async function fetchPassage(passage: string) {
    ...
    try {
      ...
      if (currentLang === 'ru') {
        ...
        // Parse chapter from passage (e.g. "Иоанн 6")
        let chapter = 1;
        if (foundBook) {
          const rest = passage.substring(foundBook.name[currentLang].length).trim();
          chapter = parseInt(rest) || 1;
        } else {
          // Fallback if book not found by name, try parsing end of string
        ...
        if (json.error || !json.info) {
          ...
          // Transform justbible.ru format to BibleResponse format
        ...
        // Use bible-api.com for ro/en
        let apiPassage = passage;
        if (foundBook) {
          const restOfPassage = passage.substring(foundBook.name[currentLang].length).trim();
          // Use ID (e.g. 'jhn') instead of full English name for better API compatibility
        ...
        if (json.error) {
          ...
          if (foundBook) {
             // Ensure we use the localized name for the reference
             // bible-api.com might return English name even if localized input was somewhat understood
             // but we specifically want our localized name.
