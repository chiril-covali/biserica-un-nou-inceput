      ...
      <div className="min-h-screen pb-20">
        ...
        <section className="py-20 md:py-28 px-6 lg:px-8 bg-[#0a2240] border-b border-white/8">
          <div className="max-w-3xl mx-auto text-center space-y-6">
            ...
            >
              <BookOpen className="size-10 mx-auto mb-4 text-sky-300/60" strokeWidth={1.2} />
              <p className="text-xs tracking-[0.3em] uppercase text-sky-400 font-bold mb-3">
                {t('bible.holy_scripture')}
              </p>
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-light tracking-wide text-white mb-4">
                {t('bible.title')}
              </h1>
        ...
        {/* Search Bar */}
        <section className="bg-[#071a2f] border-b border-white/8 py-4 px-4">
          <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-3">
            <form onSubmit={onSubmit} className="flex-1 flex gap-2 relative">
              <div className="relative flex-1 group">
                ...
                <Input
                  ...
                  className="pl-10 h-11 text-base bg-white/[0.04] border-white/10 text-white placeholder:text-white/30 focus-visible:ring-sky-400/50 focus-visible:border-sky-400/30"
                  aria-label="Search"
                />
                
                {query.length > 1 && !loading && !data && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-[#0f2d4a] border border-white/10 shadow-xl rounded-lg overflow-hidden z-50">
                    {filteredBooks.length > 0 ? (
                      filteredBooks.map(book => (
                        <button
                          ...
                          className="w-full text-left px-4 py-3 hover:bg-white/10 transition-colors border-b border-white/5 last:border-0 flex justify-between items-center text-white/80"
                        >
                          <span className="font-medium text-white">{getBookNameShort(book)}</span>
                          <span className="text-xs text-white/40">{book.chapters} {t('bible.chapters_count')}</span>
                        </button>
                      ))
                    ) : null}
                  </div>
              ...
              <Button type="submit" disabled={loading} className="h-11 px-6 shadow-sm bg-white text-[#0d2740] hover:bg-sky-100 font-bold rounded-full">
            ...
            <div className="flex gap-2">
              <Button
                ...
                className={cn("h-11 px-4 flex-1 md:flex-none border-white/10 text-white/70 hover:bg-white/10 hover:text-white", showBooks && !selectedBook && "bg-white/10 text-white")}
              >
        ...
        {/* Content */}
        <section className="px-4 sm:px-6 lg:px-8 py-10 bg-[#071a2f]">
            ...
            <AnimatePresence mode="wait">
              {showBooks && !data ? (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  className="rounded-2xl bg-white/[0.04] border border-white/8 p-6 md:p-10"
                >
                  {selectedBook ? (
                    <div className="space-y-8">
                      <div className="flex items-center gap-4 border-b border-white/8 pb-6">
                        ...
                        <h2 className="text-2xl md:text-3xl font-light text-white">{getBookNameShort(selectedBook)}</h2>
                      </div>
                      <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-3">
                        {Array.from({ length: selectedBook.chapters }, (_, i) => i + 1).map(chapter => (
                          <button
                            ...
                            className="aspect-square flex items-center justify-center text-lg border border-white/10 hover:bg-sky-500/20 hover:border-sky-400/30 hover:text-white transition-all rounded-xl font-light text-white/70"
                          >
                            {chapter}
                          </button>
                        ))}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                      <div className="space-y-4">
                        <h3 className="text-lg font-medium border-b border-white/8 pb-2 text-sky-400/70">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                          {OLD_TESTAMENT.map(book => (
                            <button
                              ...
                              className="text-left text-sm py-1.5 px-2 hover:bg-white/5 hover:text-sky-300 text-white/60 transition-colors rounded"
                            >
                ...
                <div className="space-y-8">
                  {error && (
                    <div className="p-4 border border-red-400/30 bg-red-400/5 text-red-400 text-sm rounded-lg text-center">
                      {error}
                    </div>
                  )}
                  ...
                  {data && !error && (
                    ...
                    >
                      <header className="flex flex-col md:flex-row justify-between items-center gap-6 border-b border-white/8 pb-6">
                        <Button 
                          variant="ghost" 
                          size="icon" 
                          onClick={handlePrevChapter}
                          className="size-12 rounded-full border border-white/10 hover:bg-white/10 text-white"
                          title="Anterior"
                        >
                          <ChevronLeft className="size-6" />
                        </Button>
                        ...
                        <div className="text-center group cursor-pointer" onClick={() => setShowBooks(true)}>
                          <h2 className="text-3xl md:text-4xl font-light tracking-wide text-white flex items-center gap-2">
                            {data.reference}
                            <ChevronDown className="size-5 text-white/40 group-hover:text-sky-300 transition-colors" />
                          </h2>
                        </div>
                      ...
                      <div className="space-y-6 text-lg md:text-xl font-light leading-relaxed max-w-2xl mx-auto px-4">
                        {data.verses.map((v, i) => (
                          <p key={i} className="relative pl-8 text-white/75">
                            <span className="absolute left-0 top-1 text-xs font-bold text-sky-400/40 tabular-nums">
                              {v.verse}
                            </span>
                            {v.text.trim()}
                          </p>
                      ...
                      <footer className="flex justify-center gap-4 pt-10 border-t border-white/8">
                        <Button variant="outline" onClick={handlePrevChapter} className="gap-2 font-light border-white/10 text-white/70 hover:bg-white/10 hover:text-white">
                          <ChevronLeft className="size-4" /> {t('bible.previous')}
                        </Button>
                        <Button variant="outline" onClick={handleNextChapter} className="gap-2 font-light border-white/10 text-white/70 hover:bg-white/10 hover:text-white">
